	...
	if (mysqli_num_rows(mysqli_query($con, $sql)) > 0) {
		...
		if ($id == '') {
			if ($type != 'image/jpeg' && $type != 'image/png') {
				...
				mysqli_query($con, "insert into dish(category_id,dish,dish_detail,course_id,status,added_on,image) values('$category_id','$dish','$dish_detail','$course_id',1,'$added_on','$image')");
			...
			if ($image_error == '') {
				$sql = "update dish set category_id='$category_id', dish='$dish',course_id='$course_id' , dish_detail='$dish_detail' $image_condition where id='$id'";
...
$res_course = mysqli_query($con, "select * from course where status='1' order by course")
?>
<div class="row">
					...
					<div class="form-group">
						...
						<select class="form-control" name="course_id" required>
							...
							while ($row_course = mysqli_fetch_assoc($res_course)) {
								if ($row_course['id'] == $course_id) {
									echo "<option value='" . $row_course['id'] . "' selected>" . $row_course['course'] . "</option>";
								} else {
									echo "<option value='" . $row_course['id'] . "'>" . $row_course['course'] . "</option>";
								}
							}
							?>
						</select>
